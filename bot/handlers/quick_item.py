...
def _quick_item_kb(
    ...
    *,
    ...
    if not already_tracked:
        rows.append([InlineKeyboardButton(text=tx.QUICK_ADD_BTN, callback_data=f"wbm:quick:add:{wb_item_id}")])
...
async def _quick_item_kb_with_usage(
    *,
    ...
    return _quick_item_kb(
        ...
        reviews_btn_text=tx.button_with_usage(tx.QUICK_REVIEWS_BTN, used=reviews_used, limit=reviews_limit),
        search_btn_text=tx.button_with_usage(tx.QUICK_SEARCH_BTN, used=cheap_used, limit=cheap_limit),
    )
...
def _quick_preview_text(*, product: object, already_tracked: bool) -> str:
    ...
    rating_text = (
        ...
        if rating is not None else tx.TRACK_ADDED_RATING_UNKNOWN
    )
    ...
    text = tx.QUICK_ITEM_PREVIEW_TEMPLATE.format(
        title=title, price=price_text, rating=rating_text, in_stock=in_stock_text,
    )
    if brand:
        text = f"{text}\n🏷 Бренд: {escape(brand)}"
    if already_tracked:
...
async def wb_quick_preview_cb(cb: CallbackQuery, session: "AsyncSession", redis: "Redis") -> None:
    wb_item_id = int(cb.data.split(":")[3])
    user = await get_or_create_monitor_user(session, cb.from_user.id, cb.from_user.username, redis=redis)
    ...
    await cb.message.edit_text(
        ...
        reply_markup=await _quick_item_kb_with_usage(
            ...
            user_tg_id=cb.from_user.id, user_plan=user.plan,
            wb_item_id=wb_item_id, already_tracked=bool(existing),
        ),
    )


@router.callback_query(F.data.regexp(r"wbm:quick:add:(\d+)"))
async def wb_quick_add_cb(cb: CallbackQuery, session: "AsyncSession", redis: "Redis") -> None:
    wb_item_id = int(cb.data.split(":")[3])
    user = await get_or_create_monitor_user(session, cb.from_user.id, cb.from_user.username, redis=redis)
...
async def wb_quick_reviews_cb(cb: CallbackQuery, session: "AsyncSession", redis: "Redis") -> None:
    wb_item_id = int(cb.data.split(":")[3])
    product = await fetch_product(redis, wb_item_id, use_cache=False)
    if not product:
        await cb.answer(tx.PRODUCT_FETCH_ERROR, show_alert=True)
    ...
    back_preview_kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text=tx.BTN_BACK, callback_data=f"wbm:quick:preview:{wb_item_id}")
    ]])
    ...
    cached = await QuickReviewInsightsCacheRD.get(redis, wb_item_id=wb_item_id, model_signature=model_signature)
    if cached is not None:
        insights = ReviewInsights(
            strengths=list(cached.strengths), weaknesses=list(cached.weaknesses),
            positive_samples=int(cached.positive_samples), negative_samples=int(cached.negative_samples),
            positive_total=int(cached.positive_total), negative_total=int(cached.negative_total),
        ...
        allowed, _used = await FeatureUsageDailyRD.try_consume(
            ...
            feature="reviews", limit=feature_limit, period=period, session=session,
        )
        if not allowed:
            await cb.message.edit_text(
        ...
        try:
            insights = await analyze_reviews_with_llm(
                wb_item_id=wb_item_id, product_title=product.title,
                api_key=se.agentplatform_api_key, model=model,
        ...
        await QuickReviewInsightsCacheRD(
            wb_item_id=wb_item_id, model_signature=model_signature,
    ...
    await cb.message.edit_text(
        _rit(product.title, insights) if callable(_rit) else tx.review_insights_text(product.title, insights),
        reply_markup=back_preview_kb,
...
async def wb_quick_search_cb(cb: CallbackQuery, session: "AsyncSession", redis: "Redis") -> None:
    wb_item_id = int(cb.data.split(":")[3])
    product = await fetch_product(redis, wb_item_id)
    if not product:
        await cb.answer(tx.PRODUCT_FETCH_ERROR, show_alert=True)
        return
    await cb.answer()
    await cb.message.edit_text(tx.SEARCH_MODE_PROMPT, reply_markup=_quick_search_mode_kb(wb_item_id))


@router.callback_query(F.data.regexp(r"wbm:quick:searchmode:(cheap|similar):(\d+)"))
async def wb_quick_searchmode_cb(cb: CallbackQuery, session: "AsyncSession", redis: "Redis") -> None:
    ...
    back_quick_kb = InlineKeyboardMarkup(inline_keyboard=[[
    ...
    if cached_search is not None and (mode != "cheap" or cached_search.base_price == current_price_text):
        alternatives = [
            WbSimilarItemRD(wb_item_id=item.wb_item_id, title=item.title,
                            price=item.price, url=item.url, brand=item.brand)
    ...
    if not alternatives:
        ...
        found = await search_similar_cheaper_title_only(
            base_title=product.title, base_brand=product.brand,
            base_subject_id=product.subject_id, match_percent_threshold=None,
            max_price=product.price if mode == "cheap" else Decimal("99999999"),
            exclude_wb_item_id=wb_item_id, limit=20,
        )
        live_confirmed = await _live_filter_cheaper_in_stock(
            redis, found, current_price=product.price,
            base_kind_id=product.kind_id, base_subject_id=product.subject_id,
            base_brand=product.brand, base_colors=product.colors,
            enforce_color=True, require_cheaper=(mode == "cheap"), limit=10,
            log_prefix=f"quick_id={wb_item_id} mode={mode}",
        )
        if mode == "cheap" and not live_confirmed:
            live_confirmed = await _live_filter_cheaper_in_stock(
                ...
                log_prefix=f"quick_id={wb_item_id} mode={mode} color_relaxed",
            )
            if live_confirmed:
                color_relaxed = True

        live_confirmed = _filter_candidates_by_numeric_tokens(
            base_title=product.title, candidates=live_confirmed,
        )
        alternatives = [
            WbSimilarItemRD(wb_item_id=item.wb_item_id, title=item.title,
                            price=str(item.price), url=item.url, brand=item.brand)
            for item in live_confirmed[:10]
        ]
        await QuickSimilarSearchCacheRD(
            wb_item_id=wb_item_id, mode=mode, base_price=current_price_text,
            items=[QuickSimilarItemRD(wb_item_id=item.wb_item_id, title=item.title,
                                      price=item.price, url=item.url, brand=item.brand)
    ...
    alternatives = sorted(
        alternatives,
        key=lambda x: (0 if _is_same_brand(base_brand, getattr(x, "brand", None)) else 1,
                        Decimal(str(x.price))),
    )
    header = (
    ...
    await cb.message.edit_text(
        "\n".join(lines), reply_markup=back_quick_kb,
        link_preview_options=LinkPreviewOptions(is_disabled=True),
    )
