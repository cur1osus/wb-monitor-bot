...
async def wb_search_mode_cb(cb: CallbackQuery, session: "AsyncSession") -> None:
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=tx.SEARCH_MODE_CHEAPER_BTN,
                                   callback_data=f"wbm:cheapmode:cheap:{track.id}")],
            [InlineKeyboardButton(text=tx.SEARCH_MODE_SIMILAR_BTN,
                                   callback_data=f"wbm:cheapmode:similar:{track.id}")],
            [InlineKeyboardButton(text=tx.FIND_CHEAPER_TO_LIST_BTN,
                                   callback_data=f"wbm:back:{track.id}")],
        ]
    )
    await cb.answer()
    await cb.message.edit_text(tx.SEARCH_MODE_PROMPT, reply_markup=kb)


@router.callback_query(F.data.regexp(r"wbm:cheapmode:(cheap|similar):(\d+)"))
async def wb_find_cheaper_cb(cb: CallbackQuery, session: "AsyncSession", redis: "Redis") -> None:
    ...
    back_kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text=tx.BTN_BACK, callback_data=f"wbm:cheap:{track.id}")
    ]])
    ...
    if cached is None or cached.match_percent != cfg.cheap_match_percent:
        ...
        allowed, _used = await FeatureUsageDailyRD.try_consume(
            redis, tg_user_id=cb.from_user.id,
            feature="cheap", limit=feature_limit, period=period, session=session,
        )
        if not allowed:
            await cb.answer(
                tx.FEATURE_LIMIT_CHEAP_REACHED.format(limit=feature_limit, period=period_title),
                show_alert=True,
            )
            return

        await log_event(session, track.id, "cheap_scan" if mode == "cheap" else "similar_scan",
        ...
        try:
            ...
            if _WB_ENABLE_SELENIUM_SIMILAR:
                try:
                    selenium_items = await asyncio.to_thread(
                        fetch_similar_products, track.wb_item_id,
                        limit=40, timeout_sec=20.0, headless=True,
                    )
                except Exception:
            ...
            if selenium_items:
                priced = [i for i in selenium_items if i.nm_id != track.wb_item_id and i.final_price is not None]
                ...
                reranked = [WbSimilarItemRD(wb_item_id=i.nm_id, title=i.title,
                                             price=str(i.final_price), url=i.product_url)
                             for i in selected]

            if not reranked:
                found = await search_similar_cheaper_title_only(
                    base_title=current.title or track.title, base_brand=current.brand,
                    ...
                    exclude_wb_item_id=track.wb_item_id, limit=20,
                )
                if found:
                    live_confirmed = await _live_filter_cheaper_in_stock(
                        redis, found, current_price=current.price,
                        base_kind_id=current.kind_id, base_subject_id=current.subject_id,
                        base_brand=current.brand, base_colors=current.colors,
                        enforce_color=True, require_cheaper=(mode == "cheap"), limit=20,
                    ...
                    if mode == "cheap" and not live_confirmed:
                        live_confirmed = await _live_filter_cheaper_in_stock(
                            ...
                            base_kind_id=current.kind_id, base_subject_id=current.subject_id,
                            base_brand=current.brand, base_colors=current.colors,
                            enforce_color=False, require_cheaper=True, limit=20,
                    ...
                    if mode == "similar" and len(live_confirmed) < 3:
                        relaxed = await _live_filter_cheaper_in_stock(
                            ...
                            base_kind_id=current.kind_id, base_subject_id=current.subject_id,
                            base_brand=current.brand, base_colors=current.colors,
                            enforce_color=False, require_cheaper=False, limit=20,
                    ...
                    live_confirmed = _filter_candidates_by_numeric_tokens(
                        base_title=current.title or track.title, candidates=live_confirmed,
                    )
                    if live_confirmed and len(live_confirmed) > 3:
                        llm_ranked = await rerank_similar_with_llm(
                            api_key=se.agentplatform_api_key, model=se.agentplatform_model,
                            api_base_url=se.agentplatform_base_url,
                            base_title=current.title or track.title, base_price=str(current.price),
                            base_entity=current.entity, base_subject_id=current.subject_id,
                            base_brand=current.brand, candidates=live_confirmed, limit=10,
                        )
                        reranked = [WbSimilarItemRD(wb_item_id=i.wb_item_id, title=i.title,
                                                     price=str(i.price), url=i.url)
            ...
            if not reranked:
                reranked = await _search_wb_loose_alternatives(
                    ...
                    max_price=current.price if mode == "cheap" else None, limit=8,
                )

            if reranked:
                live_input = [WbSimilarProduct(wb_item_id=i.wb_item_id, title=i.title,
                                                price=Decimal(str(i.price)), url=i.url)
                               for i in reranked]
                live_confirmed = await _live_filter_cheaper_in_stock(
                    redis, live_input, current_price=current.price,
                    base_kind_id=current.kind_id, base_subject_id=current.subject_id,
                    base_brand=current.brand, base_colors=current.colors,
                    enforce_color=True, require_cheaper=(mode == "cheap"), limit=10,
                ...
                if mode == "cheap" and not live_confirmed:
                    live_confirmed = await _live_filter_cheaper_in_stock(
                        ...
                        base_kind_id=current.kind_id, base_subject_id=current.subject_id,
                        base_brand=current.brand, base_colors=current.colors,
                        enforce_color=False, require_cheaper=True, limit=10,
                ...
                if mode == "similar" and len(live_confirmed) < 3:
                    relaxed_final = await _live_filter_cheaper_in_stock(
                        ...
                        base_kind_id=current.kind_id, base_subject_id=current.subject_id,
                        base_brand=current.brand, base_colors=current.colors,
                        enforce_color=False, require_cheaper=False, limit=10,
        ...
        await WbSimilarSearchCacheRD(
            track_id=track.id, mode=mode, base_price=current_price_text,
            match_percent=cfg.cheap_match_percent, items=alternatives,
        ).save(redis)
    else:
        await cb.answer()
    ...
    for idx, item in enumerate(alternatives, start=1):
        ...
        lines.append(f'{idx}. <a href="{item.url}">{title_text}</a> — <b>{item.price} ₽</b>')
    lines.append("")
...
async def wb_reviews_analysis_cb(cb: CallbackQuery, session: "AsyncSession", redis: "Redis") -> None:
    track_id = int(cb.data.split(":")[2])
    track = await get_user_track_by_id(session, track_id)
    if not track:
        await cb.answer(tx.TRACK_NOT_FOUND, show_alert=True)
    ...
    back_kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text=tx.REVIEWS_BACK_TO_TRACK_BTN, callback_data=f"wbm:back:{track.id}")
    ]])
    ...
    try:
        if cached is not None:
            ...
            insights = ReviewInsights(
                strengths=list(cached.strengths), weaknesses=list(cached.weaknesses),
                positive_samples=cached.positive_samples, negative_samples=cached.negative_samples,
                positive_total=cached.positive_total, negative_total=cached.negative_total,
            ...
            allowed, _used = await FeatureUsageDailyRD.try_consume(
                redis, tg_user_id=cb.from_user.id,
                feature="reviews", limit=feature_limit, period=period,
            )
            if not allowed:
                await cb.answer(
                    tx.FEATURE_LIMIT_REVIEWS_REACHED.format(limit=feature_limit, period=period_title),
                    show_alert=True,
                )
                return

            await log_event(session, track.id, "reviews_scan",
            ...
            spinner_task = asyncio.create_task(
                _progress_spinner(
                    cb.message,
                    base_text=tx.REVIEWS_ANALYSIS_PROGRESS.format(title=escape(track.title)),
                    reply_markup=back_kb,
                )
            )
            try:
                insights = await analyze_reviews_with_llm(
                    wb_item_id=track.wb_item_id, product_title=track.title,
                    api_key=se.agentplatform_api_key, model=primary_model,
                    api_base_url=se.agentplatform_base_url,
            ...
            await WbReviewInsightsCacheRD(
                wb_item_id=track.wb_item_id, model_signature=model_signature,
                strengths=list(insights.strengths), weaknesses=list(insights.weaknesses),
                positive_samples=insights.positive_samples, negative_samples=insights.negative_samples,
                positive_total=insights.positive_total, negative_total=insights.negative_total,
