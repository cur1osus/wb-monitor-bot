...
async def _show_admin_promo_list(
    ...
    if total <= 0:
        await message.edit_text(
            tx.ADMIN_PROMO_DEACTIVATE_EMPTY, reply_markup=admin_promo_kb()
        )
        return
    ...
    await message.edit_text(
        ...
        reply_markup=admin_promo_list_kb(
            items, page=safe_page, total_pages=total_pages
        ),
    )
...
@router.callback_query(AdminActionCb.filter(F.action == AdminAction.OPEN))
async def wb_admin_cb(
    cb: CallbackQuery,
    callback_data: AdminActionCb,
    ...
    await cb.message.edit_text(
        _admin_stats_text(stats), reply_markup=admin_panel_kb(selected_days=7)
    )


@router.callback_query(AdminActionCb.filter(F.action == AdminAction.CFG))
async def wb_admin_cfg_cb(
    ...
    await cb.message.edit_text(
        _admin_runtime_config_text(cfg), reply_markup=admin_config_kb()
    )


@router.callback_query(AdminActionCb.filter(F.action == AdminAction.PROMO))
async def wb_admin_promo_cb(
    cb: CallbackQuery,
    callback_data: AdminActionCb,
    state: FSMContext,
) -> None:
...
@router.callback_query(AdminActionCb.filter(F.action == AdminAction.PROMO_PRO))
async def wb_admin_promo_pro_cb(
    cb: CallbackQuery,
    callback_data: AdminActionCb,
    state: FSMContext,
) -> None:
    ...
    await cb.message.edit_text(
        tx.ADMIN_PROMO_PRO_PROMPT, reply_markup=admin_promo_input_kb()
    )


@router.callback_query(AdminActionCb.filter(F.action == AdminAction.PROMO_DISCOUNT))
async def wb_admin_promo_discount_cb(
    cb: CallbackQuery,
    callback_data: AdminActionCb,
    state: FSMContext,
) -> None:
    ...
    await cb.message.edit_text(
        tx.ADMIN_PROMO_DISCOUNT_PROMPT, reply_markup=admin_promo_input_kb()
    )


@router.callback_query(AdminActionCb.filter(F.action == AdminAction.PROMO_DEACTIVATE))
...
@router.callback_query(AdminPromoPageCb.filter())
async def wb_admin_promo_list_cb(
    cb: CallbackQuery,
    callback_data: AdminPromoPageCb,
    session: "AsyncSession",
    state: FSMContext,
) -> None:
    if not is_admin(cb.from_user.id, se):
...
@router.callback_query(AdminPromoItemCb.filter())
async def wb_admin_promo_item_cb(
    cb: CallbackQuery,
    callback_data: AdminPromoItemCb,
    session: "AsyncSession",
    state: FSMContext,
) -> None:
    if not is_admin(cb.from_user.id, se):
    ...
    await cb.message.edit_text(
        _promo_card_text(
            promo=promo, activations=activations, bot_username=bot_me.username
        ),
        reply_markup=admin_promo_card_kb(promo_id=promo.id, page=page),
    )


@router.callback_query(AdminPromoOffCb.filter())
async def wb_admin_promo_off_cb(
    cb: CallbackQuery,
    callback_data: AdminPromoOffCb,
    session: "AsyncSession",
    state: FSMContext,
) -> None:
    if not is_admin(cb.from_user.id, se):
...
@router.callback_query(AdminActionCb.filter(F.action == AdminAction.CFG_FREE))
async def wb_admin_cfg_free_cb(
    cb: CallbackQuery,
    callback_data: AdminActionCb,
    state: FSMContext,
) -> None:
    ...
    await cb.message.edit_text(
        tx.ADMIN_FREE_PROMPT, reply_markup=admin_config_input_kb()
    )


@router.callback_query(AdminActionCb.filter(F.action == AdminAction.CFG_PRO))
async def wb_admin_cfg_pro_cb(
    cb: CallbackQuery,
    callback_data: AdminActionCb,
    state: FSMContext,
) -> None:
    ...
    await cb.message.edit_text(
        tx.ADMIN_PRO_PROMPT, reply_markup=admin_config_input_kb()
    )


@router.callback_query(AdminActionCb.filter(F.action == AdminAction.CFG_CHEAP))
async def wb_admin_cfg_cheap_cb(
    cb: CallbackQuery,
    callback_data: AdminActionCb,
    state: FSMContext,
) -> None:
    ...
    await cb.message.edit_text(
        tx.ADMIN_CHEAP_PROMPT, reply_markup=admin_config_input_kb()
    )


@router.callback_query(AdminActionCb.filter(F.action == AdminAction.CFG_AI_FREE))
async def wb_admin_cfg_ai_free_cb(
    cb: CallbackQuery,
    callback_data: AdminActionCb,
    state: FSMContext,
) -> None:
    ...
    await cb.message.edit_text(
        tx.ADMIN_FREE_AI_LIMIT_PROMPT, reply_markup=admin_config_input_kb()
    )


@router.callback_query(AdminActionCb.filter(F.action == AdminAction.CFG_AI_PRO))
async def wb_admin_cfg_ai_pro_cb(
    cb: CallbackQuery,
    callback_data: AdminActionCb,
    state: FSMContext,
) -> None:
    ...
    await cb.message.edit_text(
        tx.ADMIN_PRO_AI_LIMIT_PROMPT, reply_markup=admin_config_input_kb()
    )


@router.callback_query(AdminActionCb.filter(F.action == AdminAction.CFG_REVIEWS_LIMIT))
async def wb_admin_cfg_review_limit_cb(
    cb: CallbackQuery,
    callback_data: AdminActionCb,
    state: FSMContext,
) -> None:
    ...
    await cb.message.edit_text(
        tx.ADMIN_REVIEW_SAMPLE_LIMIT_PROMPT, reply_markup=admin_config_input_kb()
    )


@router.callback_query(AdminActionCb.filter(F.action == AdminAction.CFG_ANALYSIS_MODEL))
async def wb_admin_cfg_analysis_model_cb(
    cb: CallbackQuery,
    callback_data: AdminActionCb,
    state: FSMContext,
) -> None:
    ...
    await cb.message.edit_text(
        tx.ADMIN_ANALYSIS_MODEL_PROMPT, reply_markup=admin_config_input_kb()
    )


@router.callback_query(AdminActionCb.filter(F.action == AdminAction.GRANTPRO))
async def wb_admin_grant_pro_cb(
    cb: CallbackQuery,
    callback_data: AdminActionCb,
    state: FSMContext,
) -> None:
    ...
    await cb.message.edit_text(
        tx.ADMIN_GRANT_PRO_PROMPT, reply_markup=admin_grant_pro_kb()
    )


@router.callback_query(AdminStatsCb.filter())
async def wb_admin_stats_cb(
    ...
    try:
        await cb.message.edit_text(
            _admin_stats_text(stats), reply_markup=admin_panel_kb(selected_days=days)
        )
...
async def wb_admin_promo_pro_msg(
    ...
    if parsed is None:
        await msg.answer(
            tx.ADMIN_PROMO_PRO_FORMAT_ERROR, reply_markup=admin_promo_input_kb()
        )
        return
    days, life_hours = parsed
    if days < 1 or days > 365 or life_hours < 1 or life_hours > 720:
        await msg.answer(
            tx.ADMIN_PROMO_PRO_RANGE_ERROR, reply_markup=admin_promo_input_kb()
        )
        return
    ...
    await msg.answer(
        tx.ADMIN_PROMO_CREATED_PRO.format(
            link=link, days=days, expires=expires_at.strftime("%d.%m.%Y %H:%M")
        ),
...
async def wb_admin_promo_discount_msg(
    ...
    if parsed is None:
        await msg.answer(
            tx.ADMIN_PROMO_DISCOUNT_FORMAT_ERROR, reply_markup=admin_promo_input_kb()
        )
        return
    ...
    if (
        ...
        await msg.answer(
            tx.ADMIN_PROMO_DISCOUNT_RANGE_ERROR, reply_markup=admin_promo_input_kb()
        )
        return
    expires_at = datetime.now(UTC).replace(tzinfo=None) + timedelta(hours=life_hours)
    promo = await create_promo_link(
...
async def wb_admin_grant_pro_msg(
    ...
    if parsed is None:
        await msg.answer(
            tx.ADMIN_GRANT_PRO_FORMAT_ERROR, reply_markup=admin_grant_pro_kb()
        )
        return
    ...
    if not user:
        await msg.answer(
            tx.ADMIN_GRANT_PRO_USER_NOT_FOUND, reply_markup=admin_grant_pro_kb()
        )
        return
    now = datetime.now(UTC).replace(tzinfo=None)
    base_expiry = (
    ...
    try:
        await msg.bot.send_message(
            ...
            tx.ADMIN_GRANT_PRO_USER_NOTIFY.format(
                days=days, expires=user.pro_expires_at.strftime("%d.%m.%Y %H:%M")
            ),
        )
    except Exception:
        pass
