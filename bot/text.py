...
DASHBOARD_TEMPLATE = (
    ...
    "Интервал проверок: до <b>{interval} мин</b> (адаптивно)"
)
PLAN_BADGE_PRO = "⭐ PRO"
...
DASHBOARD_HINTS: tuple[str, ...] = (
    ...
    "⚡ Можно просто отправить ссылку на товар в чат без добавления — бот покажет быстрый анализ и кнопки действий.",
    "🔔 В настройках товара можно отдельно включать/выключать уведомления о наличии и колебаниях цены.",
    "📏 Для одежды и обуви выбирай отслеживание только нужных размеров кнопками.",
    "🔎 Кнопка «Поиск» помогает найти похожие товары и сравнить варианты перед покупкой.",
    "🧠 Кнопка «Анализ отзывов» быстро собирает плюсы и минусы по развернутым отзывам.",
    "⏸ Если уведомлений слишком много — поставь товар на паузу, потом возобнови в один тап.",
    "📋 В разделе «Мои товары» можно листать карточки стрелками ⬅️➡️ и быстро прыгать по страницам через кнопку с номером страницы.",
    "⚖️ На PRO и PRO+ можно сравнивать от 2 до 20 товаров по ссылкам или артикулам и выбирать режим: дешевле, качество, подарок или без риска.",
    "🚀 Если товаров много, открой любой товар и используй кнопку с номером страницы — так быстрее перейти к нужной карточке, чем листать по одной.",
    "🧩 Быстрый режим по ссылке без добавления удобен, когда нужно посмотреть отзывы, похожие товары или сразу решить, стоит ли добавлять товар в мониторинг.",
)
...
COMPARE_ITEMS_PROMPT = (
    "⚖️ Отправьте от 2 до 20 ссылок/артикулов WB (каждый с новой строки).\n"
    "Я сравню товары, посчитаю оценки и выберу лучший вариант."
)
COMPARE_ITEMS_NOT_ENOUGH = "❌ Нужно минимум 2 валидных товара для сравнения."
COMPARE_ITEMS_TOO_MANY = "❌ За раз можно сравнить максимум 20 товаров."
COMPARE_ITEMS_PROGRESS = "⚖️ Сравниваю товары, это займет до 20 секунд..."
COMPARE_ITEMS_FAILED = "❌ Не удалось сравнить товары. Попробуйте позже."
COMPARE_ACCESS_DENIED = "🔒 Сравнение товаров доступно только на PRO/PRO+."
COMPARE_LIMIT_REACHED = (
    "⏳ Лимит сравнений на сегодня исчерпан (2/2). Попробуйте завтра."
)
...
PAYMENT_METHOD_CHOICE = (
    ...
    "Pro даёт:\n"
    "• Проверки до 60 мин (адаптивно для активных товаров)\n"
...
REFERRAL_TEXT = (
    ...
    "Тип купленной подписки не имеет значения.\n\n"
    "Ваша ссылка:\n<code>{ref_link}</code>"
)
...
HELP_TEXT = (
    ...
    "/start - Главное меню\n\n"
    "Что умеет бот:\n"
    "• отслеживать цену, наличие и остатки товаров WB\n"
    "• искать похожие или более дешевые товары\n"
    "• анализировать отзывы\n"
    "• сравнивать 2-20 товаров на PRO/PRO+\n\n"
    "Как пользоваться:\n"
    "• отправьте ссылку на товар или артикул\n"
    "• откройте «Мои товары», чтобы листать карточки стрелками ⬅️➡️\n"
    "• нажмите на кнопку с номером страницы, чтобы быстро перейти к нужному товару\n"
    "• для сравнения отправьте 2-20 ссылок/артикулов после нажатия «Сравнить товары»\n\n"
    "Если у вас есть вопросы, проблемы или предложения — нажмите кнопку ниже, чтобы написать нам."
)
...
SETTINGS_SIZES_PROMPT = (
    "📏 <b>Выбор размеров</b>\n\n"
    "Нажмите на нужные размеры ниже, затем подтвердите выбор."
)
...
REVIEW_ANALYSIS_TASK_PROMPT = (
    "Выдели до 5 сильных качеств и до 3 слабых качеств товара на основе отзывов. "
    "Не выдумывай пункты: если данных недостаточно, верни меньше пунктов или пустой список."
)
...
REVIEW_ANALYSIS_SYSTEM_PROMPT = (
    ...
    "Ограничение: strengths максимум 5, weaknesses максимум 3. Не выдумывай пункты при нехватке данных."
)
...
def review_insights_text(track_title: str, insights: "ReviewInsights") -> str:
    ...
    if insights.strengths:
        ...
        for idx, item in enumerate(insights.strengths[:max_strengths], start=1):
